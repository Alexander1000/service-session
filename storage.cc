...
SessionData* Storage::getById(std::string sessId)
{
...
int Storage::save(SessionData *sessionData)
{
...
int Storage::create(char* sessid)
{
    struct tnt_stream* conn = this->connect();
    if (conn == NULL) {
        return -1;
...
struct tnt_stream* Storage::connect()
        {
...
void Storage::free_connect(struct tnt_stream* tnt)
        {
    tnt_close(tnt);
    tnt_stream_free(tnt); // Close connection and free stream object
}

int Storage::getSpaceNo(struct tnt_stream* tnt, std::string spaceName)
{
